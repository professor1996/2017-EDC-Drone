...
// 		}
		...
		{		 
			...
			if(hs_ctrl_cnt == 0)
			{
				height_speed_ctrl(0.02f,thr, 0.4f*ultra_ctrl_out ,ultra_speed);  //0.4f*ultra_ctrl_out
			}		
			if( ultra_start_f == 0 )
			{	
...
{
	wz_speed_pid.kp = 0.4f *pid_setup.groups.hc_sp.kp;   // 0.3
	wz_speed_pid.kd = 1.5f *pid_setup.groups.hc_sp.kd;   // 1.4
	wz_speed_pid.ki = 0.12f *pid_setup.groups.hc_sp.ki;   // 原装参数0.12f
}
...
void Ultra_PID_Init()
{
	...
	ultra_pid.ki = 0;  // 0原装参数
}
 
int stop_send_ctrl3_flag = 1;
extern int move_forward_flag;
extern int Task_flag;
float exp_height_speed,exp_height = 150;   
float original_exp_height;
int   exp_height_init_flag = 1;
float ultra_speed;
float ultra_dis_lpf;
...
void Ultra_Ctrl(float T,float thr)
{
	...
	if(exp_height_init_flag)
	{
		original_exp_height = exp_height;
		exp_height_init_flag = 0;
	}
	...
	////////////////////////////////////////////////////
	if(ultra_dis_lpf > exp_height) 
	{
		start_ctrl3_flag = 1;
	}
	if(ultra_dis_lpf > exp_height + 200) 
	{
    stay_altitude_flag = 1;
	}
