...
void ili9320_Initializtion(void)
{
  ...
  ******************************/
  ...
  DeviceCode = LCD_ReadReg(0x0000);  					//read ID again
	...
	//Lcd_Light_ON;     
#ifdef ILI9325        
	LCD_WriteReg(0x00e3,0x3008);			//Y
	LCD_WriteReg(0x00e7,0x0012);			//Y
	LCD_WriteReg(0x00ef,0x1231);//Set the internal vcore voltage//Y
	LCD_WriteReg(0x0001,0x0100);//S		//Y
#endif  
#ifdef ILI9320
	...
	Delay_nms(10);
#endif        
	LCD_WriteReg(0x0002,0x0700);//Line inversion   //Y     
#if   ID_AM==000       
...
#elif ID_AM==010  
	LCD_WriteReg(0x0003,0x0010); //原来值0x0010       
#elif ID_AM==011
	LCD_WriteReg(0x0003,0x0018);			//Y
#elif ID_AM==100  
...
#endif      
	LCD_WriteReg(0x0004,0x0000);			//Y不收缩
	LCD_WriteReg(0x0008,0x0207);			//Y
	LCD_WriteReg(0x0009,0x0000);			//Y
	LCD_WriteReg(0x000a,0x0000);			//Y
	LCD_WriteReg(0x000c,0x0001);//此处配置接口类型 16位 system接口//Y
	LCD_WriteReg(0x000d,0x0000);			//Y
	LCD_WriteReg(0x000f,0x0000);    	//Y  
	//电源配置
	LCD_WriteReg(0x0010,0x0000);			//Y
//bear	LCD_WriteReg(0x0011,0x0000);
  LCD_WriteReg(0x0011,0x0007);		//	Y
	LCD_WriteReg(0x0012,0x0000);				//Y
	LCD_WriteReg(0x0013,0x0000);				//Y
	Delay_nms(200);
...
//   printf("\n\r ######********* LCD_ReadSta() = 0x%x ###### ", LCD_ReadSta());
 ...
 // 		LCD_WriteReg(0x00e7,0x0010);      
        ...
        //LCD_WriteReg(0x0003,(1<<12)|(1<<5)|(1<<4) ); 	//65K 
				LCD_WriteReg(0x0003,0x1008); 	//65K 
        LCD_WriteReg(0x0004,0x0000); 
				
        LCD_WriteReg(0x0008,0x0207);	 ////////////////不一样  
...
//bear        LCD_WriteReg(0x0092,0x0000);  
        LCD_WriteReg(0x0092,0x0600);  //////不一样
